...
function toggleRightCol() {
  ...
  const toggleButton = document.getElementById("toggleRightColButton");

  // container要素にright-col-hiddenクラスをトグル
  container.classList.toggle("right-col-hidden");

  // ボタンのテキストを更新
  if (container.classList.contains("right-col-hidden")) {
    // 右カラムが隠れている場合（左カラムが全幅）
    toggleButton.textContent = "<<"; // 右矢印
  } else {
    // 右カラムが表示されている場合
    toggleButton.textContent = ">>"; // 左矢印
  }
}
